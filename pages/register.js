...
async function callRegisterApi(action, data) {
    var res = await fetch(API_REGISTER + '?action=' + encodeURIComponent(action), {
        method: 'POST',
...
async function checkMaintenanceAndBlock() {
    try {
        var maintenanceRes = await fetch('/api/webtopupbussid?action=maintenance-status', { credentials: 'same-origin', cache: 'no-store' });
        var maintenance = maintenanceRes.ok ? await maintenanceRes.json() : null;
        if (maintenance && maintenance.maintenance) {
            tampilkanHalamanMaintenance(maintenance);
            return true;
        }
        if (!fingerprint) fingerprint = await getFingerprint();
        var blockRes = await fetch('/api/webtopupbussid?action=check-blocked', {
            credentials: 'same-origin',
            headers: { 'X-Fingerprint': fingerprint },
            cache: 'no-store'
        });
        var blocked = blockRes.ok ? await blockRes.json() : null;
...
async function register() {
    ...
    try {
        ...
        if (result && result.success) {
            ...
            if (result && (result.error === 'ip_limit' || result.error === 'fp_limit')) {
                errorMsg = result.message || 'Kamu sudah mendaftar sebelumnya. Coba lagi nanti.';
