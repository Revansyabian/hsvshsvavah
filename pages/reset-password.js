...
function storageSet(key, value) {
    try {
        ...
        allData[key] = value;
        sessionStorage.setItem('app_data', JSON.stringify(allData));
    } catch (e) {}
}
...
function storageRemove(key) {
    var allData = storageGetAll();
    delete allData[key];
    try { sessionStorage.setItem('app_data', JSON.stringify(allData)); } catch (e) {}
}

function storageGetAll() {
    try {
        var raw = sessionStorage.getItem('app_data');
...
async function periksaMaintenance() {
    try {
        var res = await fetch(API_RESET + '?action=maintenance-status', { credentials: 'same-origin', cache: 'no-store' });
        if (!res.ok) return null;
        var result = await res.json();
        return result && (result.maintenance === true || result.title || result.message) ? result : null;
    } catch (e) { return null; }
}
...
async function checkIfBlocked() {
    ...
    try {
        var res = await fetch(API_RESET + '?action=check-blocked', {
            method: 'GET',
        ...
        if (isBlocked) storageSet('perangkat_diblokir', 'true');
        else storageRemove('perangkat_diblokir');
    } catch (e) {
...
async function resetPassword() {
    ...
    try {
        ...
        var res = await fetch(API_AUTH + '?action=request-reset', {
            method: 'POST',
            credentials: 'same-origin',
            headers: {
        ...
        if (result && result.success) {
            ...
            if (result && result.error) {
                switch(result.error) {
                    case 'rate_limit':
                    case 'reset_limit':
                        errorMsg = 'Kuota reset habis. Coba lagi nanti.';
                        break;
                    case 'no_data':
