...
function encryptedTransport(req,res,next){
  ...
  res.json = (body) => {
    // Beberapa handler legacy webtopup sudah mengenkripsi payload sendiri.
    // Jangan encrypt dua kali; keluarkan envelope terluarnya saja agar
    // secure-api.js di browser bisa mendekripsi sekali dengan benar.
    if (body && body.data && body.data.v === 1 && body.data.alg === 'RSA-OAEP-256/AES-256-GCM') {
      return json(body.data);
    }
    return json(encryptResponse(body, clientKey));
  };
  next();
}
...
server.get('/admin/',(_req,res)=>res.sendFile(path.join(__dirname,'admin.html')));
server.get('/admin/register',(_req,res)=>res.sendFile(path.join(__dirname,'admin-register.html')));
